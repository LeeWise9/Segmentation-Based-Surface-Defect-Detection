# -*- coding: utf-8 -*-
"""
Created on Thu Jan 23 11:55:36 2020
...
def batch_generater(data_path, data_list, batch_size, shape, n_label):
    ...
    while True:
        ...
        Y = np.zeros((batch_size, 62, 62, n_label))
...
def show_result(img_id):
    ...
    fig  = result[0][0,...,1]
    fig1 = result[0][0,...]
    fig1 = np.reshape(fig1,(1,62*62,2))
    fig1 = np.argmax(fig1,axis=2).astype(np.int8)
    fig1 = np.reshape(fig1,(62,62))
...
if __name__ == '__main__':
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'validation'], loc='upper right')
    plt.savefig('train_val_acc_.jpg',dpi=600)
    
    show_result(np.random.randint(0,500))
